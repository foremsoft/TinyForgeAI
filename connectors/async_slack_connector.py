"""
Async Slack connector for TinyForgeAI.

Provides async functionality to list channels, get messages, and stream training
...
Usage:
    ...
    async with AsyncSlackConnector() as connector:
        ...
        messages = await connector.get_messages(channel_id="C123456")

        # Stream training samples (conversation pairs)
        async for sample in connector.stream_samples(channel_id="C123456", mapping={...}):
            print(sample)
"""
...
# Check for aiofiles library (optional, for file operations)
...
class AsyncSlackConnector:
    """
    Async Slack connector for accessing messages and streaming training samples.
    ...
    Example:
        async with AsyncSlackConnector() as connector:
            ...
            async for sample in connector.stream_samples("C123456", mapping):
                print(sample)
    """
    ...
    def __init__(self, config: Optional[SlackConfig] = None):
        """
        Initialize the async Slack connector.
        ...
        logger.debug(f"AsyncSlackConnector initialized (mock_mode={self.config.mock_mode})")

    async def __aenter__(self) -> "AsyncSlackConnector":
    ...
    async def _api_call(self, method: str, **kwargs) -> Dict[str, Any]:
        """Make an async API call to Slack."""
    ...
    async def list_channels(
        ...
        if self.config.mock_mode:
            return await self._list_channels_mock()
        return await self._list_channels_real(types, exclude_archived)

    async def _list_channels_mock(self) -> List[SlackChannel]:
        ...
        if channels_file.exists():
            data = await self._read_json_file(channels_file)
    ...
    async def get_messages(
        ...
        """
        if self.config.mock_mode:
            return await self._get_messages_mock(channel_id)
        return await self._get_messages_real(channel_id, oldest, latest, limit)

    async def _get_messages_mock(self, channel_id: str) -> List[SlackMessage]:
        ...
        if channel_dir.exists():
            for json_file in channel_dir.glob("*.json"):
                data = await self._read_json_file(json_file)
    ...
    async def get_thread_replies(
        ...
        """
        if self.config.mock_mode:
            return await self._get_thread_replies_mock(channel_id, thread_ts)
        return await self._get_thread_replies_real(channel_id, thread_ts)

    async def _get_thread_replies_mock(
        self,
        channel_id: str,
        thread_ts: str,
    ) -> List[SlackMessage]:
        ...
        if thread_file.exists():
            data = await self._read_json_file(thread_file)
            messages = data if isinstance(data, list) else data.get("messages", [])
            return [self._parse_message(m, channel_id) for m in messages]

        # Fallback: filter messages by thread_ts
        messages = await self._get_messages_mock(channel_id)
        return [m for m in messages if m.thread_ts == thread_ts or m.ts == thread_ts]

    async def _get_thread_replies_real(
        self,
        channel_id: str,
        thread_ts: str,
    ) -> List[SlackMessage]:
        ...
        while True:
            ...
            data = await self._api_call("conversations.replies", **params)
    ...
    async def get_user(self, user_id: str) -> SlackUser:
        ...
        if self.config.mock_mode:
            user = await self._get_user_mock(user_id)
        else:
            user = await self._get_user_real(user_id)

        self._users_cache[user_id] = user
        return user

    async def _get_user_mock(self, user_id: str) -> SlackUser:
        ...
        if users_file.exists():
            data = await self._read_json_file(users_file)
    ...
    async def _get_user_real(self, user_id: str) -> SlackUser:
    ...
    async def stream_samples(
        ...
        """
        if mode == "thread_qa":
            async for sample in self._stream_thread_qa(channel_id, mapping, include_reactions):
                yield sample
        elif mode == "consecutive":
            async for sample in self._stream_consecutive(channel_id, mapping, include_reactions):
                yield sample
        elif mode == "reaction_filter":
            async for sample in self._stream_reaction_filter(channel_id, mapping, include_reactions):
                yield sample
        else:
            raise ValueError(f"Unknown streaming mode: {mode}")

    async def _stream_thread_qa(
        self,
        channel_id: str,
        mapping: Dict[str, str],
        include_reactions: bool = False,
        ...
        for parent in thread_parents:
            try:
                replies = await self.get_thread_replies(channel_id, parent.ts)
    ...
    async def _stream_reaction_filter(
        self,
        channel_id: str,
        mapping: Dict[str, str],
        include_reactions: bool = False,
    ...
    async def fetch_messages_concurrent(
        self,
        channel_ids: List[str],
        max_concurrency: int = 5,
    ) -> Dict[str, List[SlackMessage]]:
        """
        Fetch messages from multiple channels concurrently.
        ...
        async def fetch_one(channel_id: str) -> tuple:
            async with semaphore:
                try:
                    messages = await self.get_messages(channel_id)
                    return (channel_id, messages)
                except Exception as e:
                    logger.warning(f"Error fetching messages for {channel_id}: {e}")
                    return (channel_id, [])

        tasks = [fetch_one(cid) for cid in channel_ids]
